...
describe('SQLMCPServer Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const configContent = TestConfigFixtures.generateConfigFileString(TestConfigFixtures.completeSetupConfig);
  ...
  describe('Server Initialization', () => {
    ...
    test('should fail initialization with invalid config', async () => {
      ...
      fs.writeFileSync(invalidConfigPath, '[database.test]\ntype = mysql\n# missing required host field');

      await expect(server.initialize(invalidConfigPath)).rejects.toThrow('missing required \'host\' field');
  ...
  describe('Error Handling and Recovery', () => {
    test('should handle server initialization failures gracefully', async () => {
      ...
      fs.writeFileSync(invalidConfig, '[database.test]\ntype = mysql\n# missing required fields like host, username');
    ...
    test('should handle connection failures gracefully', async () => {
      ...
      const mockConnectionManager = {
        ...
        hasDatabase: jest.fn().mockReturnValue(true),
        getDatabaseNames: jest.fn().mockReturnValue(['primary']),
        executeQuery: jest.fn().mockRejectedValue(new Error('No active connection found for database \'primary\''))
      };
      (server as any).connectionManager = mockConnectionManager;
