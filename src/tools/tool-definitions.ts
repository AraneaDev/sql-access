  ...
  return [
    {
      name: 'sql_query',
      description:
        'Execute a single SQL query on a configured database with automatic schema awareness and SELECT-only security enforcement. IMPORTANT: Always call sql_get_schema for the target database before your first query to learn the correct table and column names.',
      inputSchema: {
        type: 'object',
        properties: {
          database: {
    ...
    },
    {
      ...
      description:
        'Execute multiple SQL queries in batch on a configured database for improved performance. All queries must pass security validation. IMPORTANT: Always call sql_get_schema for the target database before your first query to learn the correct table and column names.',
      inputSchema: {
        type: 'object',
        properties: {
          database: {
          ...
          transaction: {
            type: 'boolean',
            description:
              'Execute all queries in a single transaction (only for full-access databases)',
            default: false,
          },
        },
    ...
    },
    {
      ...
      description:
        'List all configured databases with connection status, schema information, and security settings',
      inputSchema: {
        type: 'object',
        properties: {},
        additionalProperties: false,
    ...
    },
    {
      name: 'sql_get_schema',
      description:
        'Get detailed schema information for a database including tables, columns, relationships',
      inputSchema: {
        type: 'object',
        properties: {
          database: {
    ...
    },
    {
      ...
      description:
        'Test connection to a database (creates SSH tunnel if needed and captures schema)',
      inputSchema: {
        type: 'object',
        properties: {
          database: {
    ...
    },
    {
      ...
      description:
        'Add a new database configuration. The new database will be MCP-configurable by default.',
      inputSchema: {
        type: 'object',
        properties: {
          ...
          type: {
            type: 'string',
            enum: ['mysql', 'postgresql', 'postgres', 'sqlite', 'mssql', 'sqlserver'],
      ...
      inputSchema: {
        ...
        properties: {
          ...
          enabled: {
            type: 'boolean',
            description:
              'Must be false. Setting to true requires manual config.ini editing for security.',
          },
        },
