...
export function formatConnectionTest(result: {
  ...
  if (result.success) {
    ...
    if (result.selectOnlyMode) output += ` SELECT-only mode active\n`;
...
export function formatDatabaseList(
  ...
  for (const db of databases) {
    ...
    if (db.sslEnabled) features.push(' SSL');
    if (db.selectOnlyMode) features.push(' SELECT-only');
    ...
    if (db.schemaCached && db.schemaInfo) {
      ...
      output += ` [PENDING] Schema not yet captured\n`;
    }
