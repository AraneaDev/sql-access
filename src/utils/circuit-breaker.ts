...
export interface RejectResult { reject: true; retryInMs: number; }
export interface AllowResult { reject: false; transitionTo?: 'HALF_OPEN'; }
...
export function recordFailure(state: CircuitBreakerState, now: number): CircuitBreakerState {
  ...
  if (base.status === 'HALF_OPEN') return { status: 'OPEN', failures: 1, windowStart: now, openedAt: now };
  const failures = base.failures + 1;
  if (failures >= CIRCUIT_FAILURE_THRESHOLD) return { status: 'OPEN', failures, windowStart: base.windowStart, openedAt: now };
  return { ...base, failures };
}

export function recordSuccess(state: CircuitBreakerState): CircuitBreakerState {
  if (state.status === 'HALF_OPEN' || state.status === 'OPEN') return { status: 'CLOSED', failures: 0, windowStart: Date.now(), openedAt: null };
