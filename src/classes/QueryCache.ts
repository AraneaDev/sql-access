...
interface CacheEntry { result: QueryResult; expiresAt: number; }
interface QueryCacheOptions { maxEntriesPerDb?: number; }

function normalise(sql: string): string { return sql.replace(/\s+/g, ' ').toLowerCase().trim(); }

function makeKey(dbName: string, sql: string, params: unknown[]): string {
  return createHash('sha256').update(dbName + '\x00' + normalise(sql) + '\x00' + JSON.stringify(params)).digest('hex');
}
...
export class QueryCache {
  ...
  constructor(options: QueryCacheOptions = {}) { this.maxEntries = options.maxEntriesPerDb ?? 100; }

  shouldCache(sql: string): boolean { if (!SELECT_RE.test(sql)) return false; if (NON_DETERMINISTIC_RE.test(sql)) return false; return true; }
  isMutation(sql: string): boolean { return MUTATION_RE.test(sql); }
  ...
  get(dbName: string, sql: string, params: unknown[]): QueryResult | undefined {
    ...
    db.delete(key); db.set(key, entry); // refresh LRU
    return entry.result;
  }
  ...
  invalidate(dbName: string): void { this.store.get(dbName)?.clear(); }
}
