...
export class SetupValidator {
  ...
  static validateDatabaseName(name: string): ValidationResult {
    ...
    if (reservedNames.includes(trimmed.toLowerCase())) {
      result.warnings.push(
        `Database name '${trimmed}' is a reserved word - consider using a different name`
      );
    }
  ...
  static validateDatabaseType(type: string): ValidationResult {
    ...
    if (!validTypes.includes(normalizedType)) {
      result.isValid = false;
      result.errors.push(
        `Invalid database type '${type}'. Valid types: postgresql, mysql, sqlite, mssql`
      );
    }
  ...
  static validateHost(host: string): ValidationResult {
    ...
    if (trimmed === 'localhost' || trimmed === '127.0.0.1') {
      result.warnings.push(
        'Using localhost - ensure this is correct for your deployment environment'
      );
    }
  ...
  static validateFilePath(filePath: string): ValidationResult {
    ...
    if (!isAbsolute) {
      result.warnings.push(
        "Using relative path - ensure it's correct relative to where you run the server"
      );
    }
  ...
  static validateTimeout(timeout: string): ValidationResult {
    ...
    if (timeoutNum > 300000) {
      result.warnings.push(
        'Timeout is very long (> 5 minutes) - this may cause long waits for failed connections'
      );
    }
    ...
    try {
      ...
      if (process.platform !== 'win32') {
        ...
        if (mode & parseInt('077', 8)) {
          result.warnings.push(
            'Private key file should have restrictive permissions (600 or 400) for security'
          );
        }
      }
    } catch {
  ...
  static validateDatabaseConfig(config: Partial<DatabaseConfig>): ValidationResult {
    ...
    if (config.type === DatabaseType.SQLITE) {
      ...
      if (config.ssh_host) {
        const sshValidation = this.validateSSHConfig(
          config.ssh_host,
          config.ssh_port?.toString(),
          config.ssh_username
        );
  ...
  static validateExtensionConfig(config: Partial<ExtensionConfig>): ValidationResult {
    ...
    if (config.max_rows !== undefined) {
      const validation = this.validateNumericInput(
        config.max_rows.toString(),
        'Max rows',
        1,
        100000,
        1000
    ...
    if (config.max_batch_size !== undefined) {
      const validation = this.validateNumericInput(
        config.max_batch_size.toString(),
        'Max batch size',
        1,
        100,
        10
  ...
  static validateSecurityConfig(config: Partial<SecurityConfig>): ValidationResult {
    ...
    const validations = [
      ...
      {
        field: config.max_complexity_score,
        name: 'Max complexity score',
        min: 1,
        max: 1000,
        def: 100,
      },
      {
        field: config.max_query_length,
