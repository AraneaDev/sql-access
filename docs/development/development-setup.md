...
async function initializeDatabases() {
  console.log('🗄️  Initializing development databases...');
  
  try {
    execSync('docker-compose -f docker/development.yml up -d', { stdio: 'inherit' });
    
    // Wait for databases to be ready
    console.log('  ⏳ Waiting for databases to start...');
    await new Promise(resolve => setTimeout(resolve, 10000));
    
    console.log('  ✓ Development databases initialized');
  } catch (error) {
    console.warn('  ⚠️  Database initialization skipped (Docker may not be running)');
  }
}

async function installDevTools() {
  console.log('🛠️  Installing additional development tools...');
  ...
  try {
    ...
    console.warn('  ⚠️  Some dev tools installation skipped');
  }
}
